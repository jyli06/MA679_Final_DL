...
zm_pca <- prcomp(train[, -c(1,2)], scale = TRUE)
...
fviz_eig(zm_pca,addlabels=TRUE, hjust = -0.3)
...
```{r}
#zm_pca$x
#data <- cbind(train$Closed, zm_pca$x)
#data <- as.data.frame(data)

#model2 <- glm(V1 ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8+ PC9 + PC10 , data = data, family=binomial)
#summary(model2)

#zm_pca_test <- prcomp(test[, -c(1,2)], scale = TRUE)


#test_pca<- cbind(test$Closed, zm_pca_test$x) %>% as.data.frame()

#predicted2 <- predict(model2, test_pca, type="response")

#optimal2 <- optimalCutoff(test$Closed, predicted2)[1]

#create confusion matrix
#confusionMatrix(test$Closed, predicted2)


#calculate sensitivity
#sensitivity(test$Closed, predicted2)

#calculate specificity
#specificity(test$Closed, predicted2)


#calculate total misclassification error rate
#misClassError(test$Closed, predicted2, threshold=optimal2)
